...
usersRouter
  .post('/', jsonBodyParser, async (req, res, next) => {
    ...
    try {
      ...
      if(!ValidationService.validateEmal(email)) {
        return res.status(400).json({
    ...
    if(req.user.id !== parseInt(req.params.id)) {
      ...
      if (numberOfValues === 0) {
        return res
          .status(400)
          .json({
            error: `Request body must contain 'email', 'first_name', 'last_name', 'username', and 'password'`
          })
      }
      ...
      try{
        ...
        if(username) {
          const hasUserWithUserName = await UsersService.hasOtherUserWithUser(
            req.app.get('db'),
            username,
            id
          )
        ...
        .then(numRowsAffected => {
          ...
          .json(UsersService.serializeUser(updateUser))
          .end()
        })
      }
        catch(error) {
